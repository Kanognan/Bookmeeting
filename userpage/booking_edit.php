...
<body>
            ...
            <div class="z-3 position-absolute p-5 rounded-3"
                ...
                <nav aria-label="breadcrumb"
                    ...
                    <ol class="breadcrumb" style="margin:0 !important; color:white !important;">
                        ...
                        <li class="breadcrumb-item"><a href="booking_history.php"
                                style="color: white !important; font-size: 1em;">ประวัติการจองห้องประชุม</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page"
                            style="color: white !important; font-size: 1em;">
                            แก้ไขการจองห้องประชุม</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="container recom ">
        <!-- <h1 class="recommend card-header">แก้ไขการจองห้องประชุม</h1> -->
        <div class="row content">
            <div class="col"></div>
            <div class="col-sm-8 col-12 card-body card-body-form">
                <form action="bookingedit_model.php" class="row g-3" method="POST">
                    <?php $sql = "SELECT * FROM booking where bk_id  = $id "; ?>
                    <div class="col-md-6">
                        <label for="roomID" class="form-label">ห้องที่ต้องการจอง</label>
                        <select id="inputState" class="form-select" name="room_id">
                    ...
                    while ($row = mysqli_fetch_array($query)) {
                        ...
                        $_SESSION['id'] = $row['bk_id'];
                        ?>
                        <div class="col-md-6">
                            <label for="purpose" class="form-label">เหตุผลการใช้ห้อง</label>
                            <input type="text" class="form-control" name="purpose" value="<?php echo $row['purpose']; ?>"
                                required>
                        </div>
                        <div class="col-md-6">
                            <label for="start" class="form-label">วันเวลาเดิมที่เริ่ม</label>
                            <input type="text" class="form-control" name="start-b" id="start-b"
                                value="<?php echo $starttime; ?>" disabled>
                        </div>
                        <div class="col-md-6">
                            <label for="end" class="form-label">วันเวลาเดิมที่สิ้นสุด</label>
                            <input type="text" class="form-control" name="end-b" id="end-b" value="<?php echo $endtime; ?>"
                                disabled>
                        </div>
                        <div class="col-md-6">
                            <label for="start" class="form-label">วันเวลาใหม่ที่เริ่ม</label>
                            <input type="datetime-local" class="form-control" name="start" id="start" required>
                            <div class="form-text">จองอย่างน้อย 30 นาที และจองได้สูงสุด 3 ชั่วโมง</div>
                        </div>
                        <div class="col-md-6">
                            <label for="end" class="form-label">วันเวลาใหม่ที่สิ้นสุด</label>
                        ...
                        <div class="col-12 text-center">
                            <button type="reset" class="btn btn-cancel"
                                onclick="document.location='booking_history.php'">ยกเลิก</button>
                            <button type="submit" class="btn btn-more">แก้ไขข้อมูล</button>
                        </div>
                    <?php } ?>
                </form>
            </div>
